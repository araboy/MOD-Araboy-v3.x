		 ...
		 Else
			...
			; show normal pics
			 ...
			 GUICtrlSetState( $picResultSkippedHourNow , $GUI_ENABLE +$GUI_HIDE)
 			 ;change text
			 GUICtrlSetData($btnQuickStats,GetTranslated(13,28, "Stats"))
...
Func btnTesttrap()
	...
	Setlog(" start to search", $COLOR_PURPLE)
	$EditedImage = $hBitmap
	For $nn = 1 to $DefImages[0]
		...
		if 	UBound($position) > 3 then
			...
			for $i= 2 to (UBound($position)-2) step +2
				Local $hGraphic = _GDIPlus_ImageGetGraphicsContext($EditedImage)
				_GDIPlus_GraphicsDrawRect($hGraphic, $position[$i] , $position[$i+1] , 10, 10, $hPen)
			next
