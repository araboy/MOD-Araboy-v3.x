...
Func BrewSpells()
	...
	; Spell Creation
	; Normal Spell Factory , Auto Lightning Spell - SmartZap , will make one Spell each Loop Train until the Brew is full.

	If $ichkTrainLightSpell = 1 And $ichkSmartLightSpell = 1 Then ; Variable
		If $numFactorySpellAvaiables = 1 Then
			...
			If isSpellFactory() Then
				If $debugSetlog = 1 Then SetLog("Making Lightning Spell...SmarZap")
				If _sleep($iDelayTrain2) Then Return
				If _ColorCheck(_GetPixelColor(200, 346 + $midOffsetY, True), Hex(0x414141, 6), 20) Then
					setlog("Spell Factory Full", $COLOR_RED)
					Return
				Else
					GemClick(222, 354 + $midOffsetY, 1, $iDelayTrain7, "#0290")
					SetLog("Created 1 Lightning Spell(s)", $COLOR_BLUE)
				EndIf
			Else
