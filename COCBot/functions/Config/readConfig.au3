...
;~ 		$MilkFarmSnipeEvenIfNoExtractorsFound = IniRead($config,"MilkingAttack","TownHallHitAnyway","1")
	...
	;Chat Settings--------------------------------------------------------------------------
   $ichkGlobalChat=IniRead($config, "GlobalChat", "use","0")
   $ichkGlobalScramble=IniRead($config, "GlobalChat", "scramble","0")
   ...
   $ichkChatPushbullet=IniRead($config, "ClanChat", "pushbullet","0")
   $ichkPbSendNewChats=IniRead($config, "ClanChat", "pbsendnew","0")


   $ClanMessages = StringSplit(IniRead($config, "ClanChat", "genericMsg", "Testing on Chat|Selamat Datang di Clan|Baru Hadir bro|Assalamualaikum bro|Semoga betah ya,.."), "|", 2)
   Local $ClanResponses0 = StringSplit(IniRead($config, "ClanChat", "responseMsg", "keyword:Response|hello:Hi, Selamat Datang di clan|Hadir:Monggo|Joined:Please Intro yg baru join|Askum:waalaikum salam"), "|", 2)
