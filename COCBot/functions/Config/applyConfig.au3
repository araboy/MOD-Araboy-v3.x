...
;~ 	;Snipe While Train
   ...
   EndIf
    ...
    If $ichkGlobalChat  = 1 Then
		GUICtrlSetState( $chkGlobalChat, $GUI_CHECKED)
	Else
		GUICtrlSetState( $chkGlobalChat, $GUI_UNCHECKED)
	EndIf
    If $ichkGlobalScramble  = 1 Then
		GUICtrlSetState($chkGlobalScramble , $GUI_CHECKED)
	Else
		GUICtrlSetState( $chkGlobalScramble, $GUI_UNCHECKED)
	EndIf
    If $ichkSwitchLang  = 1 Then
		GUICtrlSetState($chkSwitchLang , $GUI_CHECKED)
	Else
		GUICtrlSetState($chkSwitchLang , $GUI_UNCHECKED)
	EndIf
    If $ichkClanChat  = 1 Then
		GUICtrlSetState($chkClanChat , $GUI_CHECKED)
	Else
	...
	EndIf
    If $ichkUseResponses  = 1 Then
		GUICtrlSetState($chkUseResponses , $GUI_CHECKED)
	Else
		GUICtrlSetState($chkUseResponses , $GUI_UNCHECKED)
	EndIf
    If  $ichkUseGeneric = 1 Then
		GUICtrlSetState($chkUseGeneric , $GUI_CHECKED)
	Else
		GUICtrlSetState( $chkUseGeneric, $GUI_UNCHECKED)
	EndIf
    If  $ichkChatPushbullet = 1 Then
		GUICtrlSetState( $chkChatPushbullet, $GUI_CHECKED)
	Else
		GUICtrlSetState($chkChatPushbullet , $GUI_UNCHECKED)
	EndIf
    If  $ichkPbSendNewChats = 1 Then
		GUICtrlSetState($chkPbSendNewChats , $GUI_CHECKED)
	Else
		GUICtrlSetState($chkPbSendNewChats , $GUI_UNCHECKED)
	EndIf
	GUICtrlSetData($editGlobalMessages1,_ArrayToString($GlobalMessages1, @CRLF))
	GUICtrlSetData($editGlobalMessages2,_ArrayToString($GlobalMessages2, @CRLF))
	GUICtrlSetData($editGlobalMessages3,_ArrayToString($GlobalMessages3, @CRLF))
	GUICtrlSetData($editGlobalMessages4,_ArrayToString($GlobalMessages4, @CRLF))
	GUICtrlSetData($editResponses,_ArrayToString($ClanResponses, ":", -1, -1, @CRLF))
