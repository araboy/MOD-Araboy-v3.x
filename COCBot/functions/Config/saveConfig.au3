...
Func saveConfig() ;Saves the controls settings to the config
	...
	If GUICtrlRead($chkChangeFF) = $GUI_CHECKED Then
		IniWrite($config, "advanced", "chkChangeFF", 1)
	Else
		IniWrite($config, "advanced", "chkChangeFF", 0)
	EndIf
...
;~     IniWrite($config, "MilkingAttack", "TownHallHitAnyway", $MilkFarmSnipeEvenIfNoExtractorsFound)
	...
	If GUICtrlRead($chkGlobalChat) = $GUI_CHECKED Then
		IniWrite($config, "GlobalChat", "use", "1")
	Else
		IniWrite($config, "GlobalChat", "use", "0")
	EndIf
	If GUICtrlRead($chkGlobalScramble) = $GUI_CHECKED Then
		IniWrite($config, "GlobalChat", "scramble", "1")
	Else
		IniWrite($config, "GlobalChat", "scramble", "0")
	EndIf
	If GUICtrlRead($chkSwitchLang) = $GUI_CHECKED Then
		IniWrite($config, "GlobalChat", "swlang", "1")
	Else
		IniWrite($config, "GlobalChat", "swlang", "0")
	EndIf
	If GUICtrlRead($chkClanChat) = $GUI_CHECKED Then
		IniWrite($config, "ClanChat", "use", "1")
	Else
	...
	Else
		IniWrite($config, "ClanChat", "simsimi", "0")
	EndIf
	If GUICtrlRead($chkUseResponses) = $GUI_CHECKED Then
		IniWrite($config, "ClanChat", "responses", "1")
	Else
		IniWrite($config, "ClanChat", "responses", "0")
	EndIf
	If GUICtrlRead($chkUseGeneric) = $GUI_CHECKED Then
		IniWrite($config, "ClanChat", "always", "1")
	Else
		IniWrite($config, "ClanChat", "always", "0")
	EndIf
	If GUICtrlRead($chkChatPushbullet) = $GUI_CHECKED Then
		IniWrite($config, "ClanChat", "pushbullet", "1")
	Else
		IniWrite($config, "ClanChat", "pushbullet", "0")
	EndIf
	If GUICtrlRead($chkPbSendNewChats) = $GUI_CHECKED Then
		IniWrite($config, "ClanChat", "pbsendnew", "1")
	Else
		IniWrite($config, "ClanChat", "pbsendnew", "0")
	EndIf
  ...
  IniWrite($config, "GlobalChat", "globalMsg1", $glb1)
   IniWrite($config, "GlobalChat", "globalMsg2", $glb2)
   IniWrite($config, "GlobalChat", "globalMsg2", $glb3)
   IniWrite($config, "GlobalChat", "globalMsg4", $glb4)
