...
Func DEDrillSearch($bReTest = False)
	...
	Local $EditedImage = _GDIPlus_BitmapCreateFromHBITMAP($hBitmapFirst)
	...
	If $listPixelByLevel[0] > 0 Then ; check for more than 1 bldg and proper split a part
	...
	If $NumDEDrill <> 0 Then
		For $i = 0 To $NumDEDrill
			...
			If isInsideDiamond($pixel) Then
				...
				If $debugImageSave = 1 Then
					$ImageInfo = String("Lv_" & $level)
					_GDIPlus_GraphicsDrawRect($hGraphic, $aDrills[$i][0] - 5, $aDrills[$i][1] - 5, 10, 10, $hPen)
					_GDIPlus_GraphicsDrawString($hGraphic, $ImageInfo, $aDrills[$i][0] + 10, $aDrills[$i][1] - 15, "Arial", 15)
				EndIf
		...
		Next
		If $debugImageSave = 1 Then
			$ImageInfo = String("c# T:" & Round(TimerDiff($hTimerTest) / 1000, 2) & "´s")
			_GDIPlus_GraphicsDrawString($hGraphic, $ImageInfo, 400, 63, "Arial", 15)
			Local $filename = String("c#_DrillsDetected_" & $Date & "_" & $Time & ".png")
			_GDIPlus_ImageSaveToFile($EditedImage, $dirTempDebug & $filename)
			SetLog("Total No. of Dark Elixir Drills = " & $NumDEDrill, $COLOR_FUCHSIA)
			SetLog("  - Calculated  in: " & Round(TimerDiff($hTimerTest) / 1000, 2) & " seconds ", $COLOR_TEAL)
		EndIf
	EndIf
